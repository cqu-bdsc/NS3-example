...
//      positionAlloc2->Add (Vector ((i % nInLine) * 100.0, obu1Y, 0.0));
      positionAlloc2->Add (Vector (i * 500.0, obu1Y, 0.0));
    }
...
//    }
  positionAlloc3->Add (Vector (850.0, 0.0, 0.0));
  mobility.SetPositionAllocator (positionAlloc3);
  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
...
//    }
  ...
  for (uint32_t u = 0; u < rsuNodes.GetN (); ++u)
    {
      PacketSinkHelper rsuPacketSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (rsu_iface_80211p.GetAddress(u), v2iPort));
      serverApps.Add (rsuPacketSinkHelper.Install (rsuNodes.Get(u)));
    }
...
#if 0
  for (uint32_t i = 1; i < obuNodes.GetN(); ++i)
    {
      Ptr<Socket> obuSource = Socket::CreateSocket (obuNodes.Get(i), tid);
      InetSocketAddress obuRemote = InetSocketAddress (obu_iface_80211p.GetAddress(i), v2vPort);
...
#if 1
  {
    ...
    InetSocketAddress remote = InetSocketAddress (rsu_iface_80211p.GetAddress(0), v2iPort);
    source->Connect (remote);
    Simulator::Schedule(Seconds(1), &SendData, source, "1");
  }
  {
    Ptr<Socket> source = Socket::CreateSocket (rsuNodes.Get(1), tid);
    InetSocketAddress remote = InetSocketAddress (obu_iface_80211p.GetAddress(1), v2vPort);
    source->Connect (remote);
    Simulator::Schedule(Seconds(1), &SendData, source, "2");
  }
  {
    Ptr<Socket> source = Socket::CreateSocket (obuNodes.Get(2), tid);
    InetSocketAddress remote = InetSocketAddress (rsu_iface_80211p.GetAddress(2), v2iPort);
    source->Connect (remote);
    Simulator::Schedule(Seconds(1), &SendData, source, "3");
  }
#endif
...
//    }
    ...
    dlSource->SetAllowBroadcast(true);
    dlSource->Connect (dlRemote);
    Simulator::Schedule(Seconds(2), &SendData, dlSource, "lte-test-dl");
#endif
