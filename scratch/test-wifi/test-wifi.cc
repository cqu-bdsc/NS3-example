...
//  std::string socketType ("ns3::PacketSocketFactory");
  ...
  cmd.AddValue ("nPkts", "number of packets generated", numPackets);
...
//  positionAlloc->Add (Vector (1500, 1500 - distance, 0.0));

  for (uint32_t i = 0; i < obuNodes.GetN (); ++i)
    {
      positionAlloc->Add (Vector (1500 + i, 1500 - distance, 0.0));
    }
  ...
  for (uint32_t i = 0; i < nObu; ++i)
    {
      ...
      InetSocketAddress remote = InetSocketAddress (interface_80211_rsu.GetAddress (0), 80);
      source->Connect (remote);
      Simulator::Schedule(Seconds(0.01 + i * 0.001), &SendData, source);
    }
#endif
